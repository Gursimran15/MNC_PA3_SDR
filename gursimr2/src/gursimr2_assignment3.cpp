...
//     return bytes;
// }
// // ssize_t	recvALLfromip(int sock_index, char *buffer, ssize_t nbytes,sockaddr *dest_addr, socklen_t *addrlen){
// // ssize_t bytes = 0;
// //     bytes = recvfrom(sock_index, buffer, nbytes, 0,dest_addr,addrlen);
...
//             tmp=rt_init[i].router_id;
// 			memcpy(dv_payload + o,&tmp,2);
// 			o+=2;
//             cout<<tmp<<" rt init rid\n";
// 			cout<<ntohs(tmp)<<"\n";
...
// }
// // void dv_recv(int sock){
// //     uint16_t payload_len = ((sizeof(uint16_t))*4 + (sizeof(uint32_t)))*NUM_ROUTERS + 2*(sizeof(uint16_t)) + (sizeof(uint32_t));
// //     sockaddr router_add;
// //     //  getpeername(sock, &router_add,(sizeof(router_add)));
// // 	recvALLfromip(sock,dv_payload,payload_len,(struct sockaddr *)&router_add,(socklen_t *)sizeof(router_add));
// //     cout<<"\n\n\n\nReceived DV Update\n\n\n\n";
// //     LOG_PRINT("Inside Neighbour if");
// //      LOG_PRINT("Inside dv recv for");
// // LOG_PRINT("Dv recv done");
...
void dv_recv(int sock){
    uint16_t payload_len = ((sizeof(uint16_t))*4 + (sizeof(uint32_t)))*NUM_ROUTERS + 2*(sizeof(uint16_t)) + (sizeof(uint32_t));
    sockaddr router_add;
    char *dv_payload_response;
    //  getpeername(sock, &router_add,(sizeof(router_add)));
	recvALLfromip(sock,dv_payload_response,payload_len,(struct sockaddr *)&router_add,(socklen_t *)sizeof(router_add));
    cout<<"\n\n\n\nReceived DV Update\n\n\n\n";
    // LOG_PRINT("Inside Neighbour if");
    //  LOG_PRINT("Inside dv recv for");
LOG_PRINT("Dv recv done");
